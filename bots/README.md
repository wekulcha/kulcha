...
## Мини-приложение по HTTPS (Tunneler)

Проверка **initData** для мини-аппа — в **`auth_gateway`** (Python), бизнес-логика — в Spring (Java). Переменные окружения — см. **`scripts/kulcha-dev.env.example`**.
