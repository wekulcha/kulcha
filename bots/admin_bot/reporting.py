...
def _section_lines(section: dict[str, Any], title: str) -> list[str]:
    lines = [
        ...
        f"Сумма блюд: {_money(section.get('itemsTotal'))}",
    ]
...
def build_today_report_messages(summary: dict[str, Any]) -> list[str]:
    ...
    header = [
        ...
        "━━━━━━━━━━━━━━",
        f"Дата отчёта: {html.escape(str(report_date))} ({html.escape(str(timezone))})",
        f"Ресторанов в отчёте: {len(restaurants)}",
    ]
    messages.append("\n".join(header))

    for restaurant in restaurants:
        lines = [
            f"<b>{html.escape(str(restaurant.get('restaurantName') or 'Ресторан'))}</b>",
            "━━━━━━━━━━━━━━",
        ...
        lines.extend(_section_lines(restaurant.get("dineIn") or {}, "Заказы в зале"))
        lines.append("")
...
def build_today_report_pdf(summary: dict[str, Any]) -> bytes | None:
    ...
    for index, restaurant in enumerate(restaurants):
        story.append(
            Paragraph(
                html.escape(str(restaurant.get("restaurantName") or "Ресторан")),
                heading_style,
            )
        )
        story.append(
        ...
        for title, key in (("Заказы в зале", "dineIn"), ("Доставка", "delivery")):
